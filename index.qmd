...
```{r}
p2 <- transform(data.frame(x=c(0:10)) , y=dpois(x, 2))
p05 <- transform(data.frame(x=c(0:10)) , y=dpois(x, 0.5))
p4 <- transform(data.frame(x=c(0:10)) , y=dpois(x, 4))

p1 <- ggplot2::ggplot() + 
        ggplot2::lims(y=c(0,1))+
          ggplot2::geom_area(ggplot2::aes(x, y),p05,alpha = .5,fill = "blue" )+
            ggplot2::geom_area(ggplot2::aes(x, y),p4,alpha = .5,fill = "black" )+
              ggplot2::geom_area(ggplot2::aes(x, y),p2,alpha = .5,fill = "purple")+
                ggplot2::ggtitle("Distribuições Poisson") +
                 ggplot2::annotate("text",y = 1,x = 7.5,label = "lambda igual a 0,5",color= "blue",)+
                 ggplot2::annotate("text",y = 0.9,x = 7.5,label = "lambda igual a 4",color= "black")+
                 ggplot2::annotate("text",y = 0.8,x = 7.5,label = "lambda igual a 2",color= "purple")+
      see::theme_modern()
...
```{r}
p2 <- transform(data.frame(x=c(0:10)) , y=dpois(x, 2))
b2 <- transform(data.frame(x=c(0:10)) , y=dnbinom(x,2,1/2))
p10 <- transform(data.frame(x=c(0:10)) , y=dpois(x, 10))
b10 <- transform(data.frame(x=c(0:10)) , y=dnbinom(x,10,1/2))
 ggplot2::ggplot() + 
        ggplot2::lims(y=c(0,.35))+
          ggplot2::geom_area(ggplot2::aes(x, y),p2,alpha = .5,fill = "blue" )+
            ggplot2::geom_area(ggplot2::aes(x, y),b2,alpha = .5,fill = "black" )+
             ggplot2::geom_area(ggplot2::aes(x, y),p10,alpha = .5,fill = "blue" )+
               ggplot2::geom_area(ggplot2::aes(x, y),b10,alpha = .5,fill = "black" )+
                ggplot2::ggtitle("Distribuições Poisson") +
      see::theme_modern()
